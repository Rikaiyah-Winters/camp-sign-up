describe('Fill out form and Check List', () => {
  it('Has filled out the form and checked the list', () => {
    ...
    cy.get('input[name="name"]').type('Snowy Simpson', {delay: 100}) //create camper
    cy.get('input[name="age"]').type('12')
    cy.get('input[name="number"]').type('601-630-8004', {delay: 100})
    cy.get('input[name="email"]').type('coltsfan13@gmail.com', {delay: 100})
    ...
    cy.get('input[name]').type('snowy simpson')//assuring that this is case sensetive
    cy.get('form').submit()
    cy.get('a[href="/"]').click()
  })
})